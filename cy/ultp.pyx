...
def go(params: PD):
    ...
    cdef double pipe_vol = np.pi * pipe_in_rad ** 2.0 * (zM - z0)
    with open(wrt_dir+'filled_array_'+str(x_res)+'-'+str(y_res)+'-'+str(z_res)+'.npy', 'rb') as f:
        loaded = np.load(f)
        f.close()

    avg_z = []
    cdef double dV = dx*dy*dz
    cdef double total_vol_z = np.pi * pipe_in_rad ** 2.0 * dz
    cdef double sumz,fill_vol_z,p_z = 0.0
    for z in range(0,z_res):
        ...
        if z%100==0:
            print("completed z =",z)
        fill_vol_z = sumz * dV
        p_z = (total_vol_z-fill_vol_z)/total_vol_z
        avg_z.append(p_z)
    ...
    avg_rs = np.zeros(r_res+1)
    rs = np.linspace(0,pipe_in_rad,len(avg_rs))
    cdef double dr = pipe_in_rad/(r_res+1.0)
    cdef double pr_sq = pipe_in_rad**2.0
    oops_count = 0
    zstart = int(round((z0_cutoff-z0)/dz))
    print("zstart:",zstart)
    zend = int(round((zM_cutoff-z0)/dz))
    print("znd:",zend)
    for z in range(zstart,zend):
        if (z-zstart)%int((zend-zstart)*0.1)==0:
            print('%d'.format(round(100*(z-zstart)/(zend-zstart))) + "%" +" complete")

        for x in range(x_res+1):
            x_phys = abs(x*dx+x0) #physical coordinate of the center of this node
            if abs(x_phys)>pipe_in_rad:
                print("big x: ",(x_phys-pipe_in_rad)/dr)
            if x_phys >= pipe_in_rad:
            ...
            for y in range(y_min_li,y_max_li):
                y_phys = abs(y*dy+y0)
                if abs(y_phys) >= pipe_in_rad:
                    avg_rs[r_res] += loaded[x][y][z]
                    continue
                r_phys = np.sqrt(x_phys**2+y_phys**2)
                #now scatter to radii outward and inward (linear extrapolation)
                r_node_in = 0
                r_node_out = r_res
                if (r_phys==0.0):
